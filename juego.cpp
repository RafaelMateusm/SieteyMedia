...
#include<time.h>
using namespace std;
// funiones necesarias para el juego
const int tam = 10;
...
bool juego(int opcion)             //función juego
{
	...
	if (archivoAbierto.is_open() && ((opcion == 1) || (opcion == 2)))
	{
		...
		if (opcion == 1)
		{
			puntosHumano = modoA(archivoAbierto, numCartas);            // llama a la función modoA
			if (puntosHumano > 7.5)
			{
			...
			else
			{
				...
				puntosMaquina = modoA(archivoAbierto, numCartas);            // llama a la función modoA para el turno de la máquina
				...
				{
					determinaGanador(puntosHumano, puntosMaquina); // llama a la función determinaGanador para poder determinar el ganador
				}
			}
		}
		if (opcion == 2)
		{
			puntosHumano = modoBhumano(archivoAbierto, numCartas);     // llama a la función modoBhumano
			if (puntosHumano > 7.5)
			{
				cout << "Ha ganado la máquina :( " << endl;
			}
			...
			{
				cout << "Es el turno de la máquina" << endl;
				puntosMaquina = modoBmaquina(archivoAbierto, numCartas, puntosHumano);   // llama la función modoBmáquina
				if (puntosMaquina > 7.5)
				{
					cout << "¡¡ERES EL GANADOR!!" << endl;
				}
				...
				{
					determinaGanador(puntosHumano, puntosMaquina);        // llama a la función determinaGanador
				}
			}
		}
	}
...
y devuelvo los puntos obtenidos tras robar ese número de cartas.*/
...
Recibe el archivo con el mazo y el número máximo de cartas que puede robar, y devuelve los puntos obtenidos.*/
...
/*Permite realizar el turno del jugador máquina en el modo B.
Recibe el archivo con el mazo, el número máximo de cartas que puede robar y la puntuación obtenida por el jugador humano,
...
/*Recibe los puntos obtenidos por el jugador humano y por la máquina, y devuelve un valor que indica quién gana.*/
int determinaGanador(float puntosHumano, float puntosMaquina)
{
	int ganador;
